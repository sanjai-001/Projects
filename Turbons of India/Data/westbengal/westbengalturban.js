...
function preload()
{
  ...
  imgturban = loadImage("https://i.ibb.co/0QsJzX3/BENGAL-1.png");
}
...
function drawturban()
{
  ...
  {
    push();
    const wx = Math.abs(positions[13][0] - positions[1][0]) * 2.7; // The width is given by the face width, based on the geometry
    const wy = Math.abs(positions[7][1] - Math.min(positions[16][1], positions[20][1])) * 2.7; // The height is given by the distance from nose to chin, times 2
    translate(-wx/1.9,-wy/1.05);
